...
if (!$enclosure) {
    header("Location: pet_enclosures.php?error=1&msg=" . urlencode("ID lưu chuồng không hợp lệ!"));
    exit;
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!empty($customer_id) && !empty($pet_id) && !empty($enclosure_number) && !empty($check_in_date) && !empty($daily_rate) && !empty($status)) {
        $updated = updatePetEnclosure($enclosure_id, $customer_id, $pet_id, $enclosure_number, $check_in_date, $check_out_date, $daily_rate, $deposit, $emergency_limit, $note, $status);
        if ($updated) {
            header("Location: pet_enclosures.php?success=2&msg=" . urlencode("Cập nhật chuồng thành công!"));
            exit;
        } else {
            header("Location: pet_enclosures.php?error=1&msg=" . urlencode("Cập nhật chuồng thất bại!"));
            exit;
        }
    } else {
...
<head>
    ...
    <title>Sửa chuồng thú - Spa Thú Cưng Min Min</title>
...
<body>
            ...
            <div class="form-container">
                ...
                <form action="" method="post">
                    <div class="form-group">
                        ...
                        <select name="customer_id" id="customer_id" required>
                            ...
                            <?php foreach ($customers as $c): ?>
                                <option value="<?= $c['customer_id'] ?>" <?= $c['customer_id'] == $enclosure['customer_id'] ? 'selected' : '' ?>>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="pet_enclosure_number" id="pet_enclosure_number" value="<?= $enclosure['pet_enclosure_number'] ?>" min="0" required>
                    </div>

                    <div class="form-group">
                        <label for="check_in_date">Check-in <span class="required-field">*</span></label>
                        <input type="datetime-local" name="check_in_date" id="check_in_date"
                            value="<?= date('Y-m-d\TH:i', strtotime($enclosure['check_in_date'])) ?>" required>
                    </div>

                    <div class="form-group">
                        <label for="check_out_date">Check-out</label>
                        <input type="datetime-local" name="check_out_date" id="check_out_date"
                            value="<?= $enclosure['check_out_date'] ? date('Y-m-d\TH:i', strtotime($enclosure['check_out_date'])) : '' ?>">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="daily_rate" id="daily_rate" value="<?= $enclosure['daily_rate'] ?>" min="0"
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="deposit" id="deposit" value="<?= $enclosure['deposit'] ?>" min="0">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="emergency_limit" id="emergency_limit">
                            ...
                            <option value="500000" <?= $enclosure['emergency_limit'] == 500000 ? 'selected' : '' ?>>500.000 ₫</option>
                            <option value="1000000" <?= $enclosure['emergency_limit'] == 1000000 ? 'selected' : '' ?>>1.000.000 ₫</option>
                            <option value="3000000" <?= $enclosure['emergency_limit'] == 3000000 ? 'selected' : '' ?>>3.000.000 ₫</option>
                        </select>
                    </div>

                    <div class="form-group">
                        <label for="pet_enclosure_note">Ghi chú</label>
                        <textarea name="pet_enclosure_note" id="pet_enclosure_note" rows="3"><?= htmlspecialchars($enclosure['pet_enclosure_note']) ?></textarea>
                    </div>
    ...
    <script>
        ...
        const currentPetId = <?= (int)$enclosure['pet_id'] ?>;
        ...
        // Load sẵn thú cưng khi edit
        loadPets(<?= (int)$enclosure['customer_id'] ?>);
    </script>
</body>
