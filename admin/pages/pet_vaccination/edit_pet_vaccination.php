...
$pet_vaccination_id = intval($_GET['id']);
$record = getPetVaccinationById($pet_vaccination_id);
if (!$record) {
    header("Location: pet_vaccinations.php?error=1&msg=" . urlencode("ID lịch tiêm không hợp lệ!"));
    exit;
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!empty($vaccine_id) && !empty($customer_id) && !empty($pet_id) && !empty($doctor_id) && !empty($vaccination_date)) {
        if (updatePetVaccination($pet_vaccination_id, $vaccine_id, $customer_id, $pet_id, $doctor_id, $vaccination_date, $next_vaccination_date, $notes)) {
            header("Location: pet_vaccinations.php?success=2&msg=" . urlencode("Cập nhật lịch tiêm vaccine thành công!"));
            exit;
        } else {
            header("Location: pet_vaccinations.php?error=1&msg=" . urlencode("Cập nhật lịch tiêm vaccine thất bại!"));
            exit;
        }
    } else {
...
<head>
    ...
    <title>Chỉnh sửa lịch tiêm vaccine - Spa Thú Cưng Min Min</title>
...
<body>
    ...
    <main class="main">
        ...
        <main class="content">
            <div class="form-container">
                <h2>Chỉnh sửa lịch tiêm vaccine</h2>
                ...
                <form action="" method="post">
                    ...
                    <div class="form-group">
                        ...
                        <select name="customer_id" id="customer_id" required>
                            ...
                            <?php foreach ($customers as $c): ?>
                                <option value="<?php echo $c['customer_id']; ?>" <?= $c['customer_id'] == $record['customer_id'] ? 'selected' : '' ?>>
                    ...
                    <div class="form-group">
                        ...
                        <select name="vaccine_id" id="vaccine_id" required>
                            ...
                            <?php foreach ($vaccines as $v): ?>
                                <option value="<?php echo $v['vaccine_id']; ?>" <?= $v['vaccine_id'] == $record['vaccine_id'] ? 'selected' : '' ?>>
                    ...
                    <div class="form-group">
                        ...
                        <select name="doctor_id" id="doctor_id" required>
                            ...
                            <?php foreach ($doctors as $d): ?>
                                <option value="<?php echo $d['doctor_id']; ?>" <?= $d['doctor_id'] == $record['doctor_id'] ? 'selected' : '' ?>>
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" name="vaccination_date" id="vaccination_date" value="<?= date('Y-m-d', strtotime($record['vaccination_date'])) ?>" required>
                    </div>

                    <!-- Ngày tiêm tiếp theo -->
                    <div class="form-group">
                        <label for="next_vaccination_date">Ngày tiêm tiếp theo</label>
                        <input type="date" name="next_vaccination_date" id="next_vaccination_date" value="<?= $record['next_vaccination_date'] ? date('Y-m-d', strtotime($record['next_vaccination_date'])) : '' ?>">
                    </div>

                    <!-- Ghi chú -->
                    <div class="form-group">
                        <label for="notes">Ghi chú</label>
                        <textarea name="notes" id="notes" rows="3"><?= htmlspecialchars($record['notes'] ?? '') ?></textarea>
                    </div>
    ...
    <script>
        // Lọc thú cưng theo chủ nuôi (giữ logic giống add_pet_vaccination.js)
        const pets = <?php echo json_encode($pets); ?>;
        const customerSelect = document.getElementById("customer_id");
        const petSelect = document.getElementById("pet_id");
        const currentPetId = <?= (int)$record['pet_id'] ?>;
        ...
        function loadPets(customerId) {
            ...
            pets.filter(p => p.customer_id == customerId)
                .forEach(p => {
                    ...
                    if (p.pet_id == currentPetId) opt.selected = true;
        ...
        // Load sẵn thú cưng tương ứng chủ nuôi hiện tại
        <?php if (!empty($record['customer_id'])): ?>
            loadPets(<?= (int)$record['customer_id'] ?>);
        <?php endif; ?>
    </script>
</body>
