...
$treatment_course_id = $_GET['id'];
$treatment_course = getTreatmentCourseById($treatment_course_id);

if (!$treatment_course) {
    header("Location: treatment_courses.php?error=1&msg=" . urlencode("ID đợt khám không hợp lệ!"));
    exit;
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!empty($customer_id) && !empty($pet_id) && !empty($start_date)) {
        $updated = updateTreatmentCourse($treatment_course_id, $customer_id, $pet_id, $start_date, $end_date, $status);

        if ($updated) {
            header("Location: treatment_courses.php?success=1&msg=" . urlencode("Cập nhật đợt khám thành công!"));
            exit;
        } else {
            header("Location: treatment_courses.php?error=1&msg=" . urlencode("Cập nhật đợt khám thất bại!"));
            exit;
        }
    } else {
...
<head>
    ...
    <title>Sửa đợt khám - Spa Thú Cưng Min Min</title>
...
<body>
                ...
                <form action="" method="post">
                        ...
                        <select name="customer_id" id="customer_id" required>
                            ...
                            <?php foreach ($customers as $c): ?>
                                <option value="<?= $c['customer_id'] ?>"
                                    <?= $c['customer_id'] == $treatment_course['customer_id'] ? 'selected' : '' ?>>
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" name="start_date" id="start_date"
                            value="<?= htmlspecialchars($treatment_course['start_date']) ?>" required>
                    </div>

                    <div class="form-group">
                        <label for="end_date">Ngày kết thúc</label>
                        <input type="date" name="end_date" id="end_date"
                            value="<?= htmlspecialchars($treatment_course['end_date'] ?? '') ?>">
                    </div>

                    <div class="form-group">
                        <label for="status">Trạng thái <span class="required-field">*</span></label>
                        <select name="status" id="status" required>
                            <option value="1" <?= $treatment_course['status'] == 1 ? 'selected' : '' ?>>Đang điều trị</option>
                            <option value="0" <?= $treatment_course['status'] == 0 ? 'selected' : '' ?>>Kết thúc</option>
                        </select>
                    </div>
    ...
    <script>
        ...
        const treatmentPetId = <?= isset($treatment['pet_id']) ? (int)$treatment['pet_id'] : 'null' ?>;
        ...
        function loadPets(customerId) {
            ...
            pets.filter(p => p.customer_id == customerId).forEach(p => {
                ...
                if (treatmentPetId && p.pet_id == treatmentPetId) opt.selected = true;
        ...
        <?php if (!empty($treatment['customer_id'])): ?>
            loadPets(<?= (int)$treatment['customer_id'] ?>);
        <?php endif; ?>
    </script>
